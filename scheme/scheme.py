...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        prob_operator = scheme_eval(first, env)
        prob_operands = rest.map(lambda oper: scheme_eval(oper, env))
        return scheme_apply(prob_operator, prob_operands, env)
        
        
        '''prob_operands = rest.map(lambda procedure: scheme_apply(procedure, rest, env))'''
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        in a Spython3 ok -q 03 -ucheme list of formal parameters FORMALS are bound to the Scheme
