...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        ghostPositions = []
        for LGPDist in livingGhostPositionDistributions:
            ghostPositions.append(LGPDist.argMax())
        distances = []
        for ghostPos in ghostPositions:
            distances.append(self.distancer.getDistance(pacmanPosition, ghostPos))
        closestPos = ghostPositions[distances.index(min(distances))]
        minDistance = 10000
        retAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            resultDistance = self.distancer.getDistance(successorPosition, closestPos)
