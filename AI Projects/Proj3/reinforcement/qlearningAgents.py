    ...
    def computeActionFromQValues(self, state):
        ...
        for action in actions:
          ...
          if( value > max):
            ...
            max = value
          if( value == max):
            result = random.choice([action, ret_action])
            if( not result == ret_action):
              ret_action = action
              max = value
        return ret_action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature, value in feat.items():
          sum += value * self.weights[feature]
        return sum
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature, value in feat.items():
           curr_weight = self.weights[feature]
           self.weights[feature] = curr_weight + self.alpha * difference * value
