...
def swine_align(player_score, opponent_score):
    ...
    if opponent_score > player_score:
      start_divisor = player_score
    else:
      start_divisor = opponent_score

    while start_divisor >= 10:
      if player_score % start_divisor == 0 and opponent_score % start_divisor == 0:
        return True
      start_divisor -= 1
    return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    score_gain = 0
    turn_counter = 1
    extra_turn_bool = False
    while max(score0, score1) < goal:
      if who == 0:
        num_rolls_0 = strategy0(score0, score1)
        score_gain = take_turn(num_rolls_0, score1, dice)
        score0 += score_gain
        if turn_counter == 1:
          say_func = say(score0, score1)
          turn_counter += 1
        else:
          say_func = say_func(score0, score1)
        extra_turn_bool = extra_turn(score0, score1)
      else:
        num_rolls_1 = strategy1(score1, score0)
        score_gain = take_turn(num_rolls_1, score0, dice)
        score1 += score_gain
        say_func = say_func(score0, score1)
        extra_turn_bool = extra_turn(score1, score0)
      if not extra_turn_bool:
        who = other(who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    greatest_average = 0
    while turn <= 10:
      average = average_roll(turn , dice)
      if average > greatest_average:
        greatest_average = average
        greatest_average_turn = turn
      turn += 1
    return greatest_average_turn
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if swine_align(bacon_score, opponent_score):
      return 0
    elif pig_pass(bacon_score, opponent_score):
      return 0
