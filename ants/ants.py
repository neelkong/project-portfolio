...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while curr_place is not beehive:
            if curr_place.bees and self.min_range <= distance <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            death_dmg = self.damage
        i = len(bee_list) - 1
        while i >= 0:
            Insect.reduce_armor(bee_list[i], amount + death_dmg)
            i-=1
        # END Problem 5
...
# END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if amount >= self.armor and self.true_queen:
            bees_win()
        else:
